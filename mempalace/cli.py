...
def cmd_export(args):
    """Export palace data — auto-detects JSON/JSONL from output path."""
...
def main():
    ...
    p_export.add_argument(
        ...
        default="auto",
        help="Format override (default: auto-detect from output path)",
    )

    # import
    p_import = sub.add_parser(
        "import",
        help="Import palace (auto: .json file or directory of JSONL files)",
    )
    ...
    p_backup.add_argument("--zip", action="store_true", help="Create a zip archive instead")
    p_backup.add_argument(
        "--max-backups", type=int, default=5, help="Max backups to retain (0 = unlimited)"
    )
